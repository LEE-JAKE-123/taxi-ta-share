-- FR-33~40 / TR-01~03: 0039 preserved a legacy full-escrow path without an
-- immutable historical fare snapshot.  A positive deposit must therefore be
-- checked against the current exact share; otherwise an untracked partial
-- escrow could be confirmed without an OPEN shortfall.
...
BEGIN
  ...
  FROM trip_participants p WHERE trip_id = NEW.trip_id;
